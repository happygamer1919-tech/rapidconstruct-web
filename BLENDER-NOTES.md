...
## ⛔ STALE — Verified state of HomeRC.blend (2026-07-15) — DO NOT USE
> **This section is SIX+ sessions out of date and caused a dead-hypothesis research sprint
> (2026-07-21: "13 flat materials" was read from here and was wrong by then).**
> CURRENT state (2026-07-21 19:16, = HomeRC_backup_surfaceD.blend): ~330 objects / 326 meshes,
> 26+ materials (incl. clad_dark, wall_fence, stone_fence, foliage_tree a/b/c, grey_plaster,
> aerial_grass_rock), 15+ images, Look B cladding + greige walls + jerkinhead + stage-D surface
> experiments. ALWAYS verify live (`len(bpy.data.objects)`, glb inspect) instead of trusting
> any scene dump below.
...
## SURFACE-REALISM RESEARCH SPRINT — 2026-07-21 (RESEARCH ONLY: no rebuild, no ship;
## pristine house.glb restored, checksum-verified OK)
- **Hypothesis verdict**: main session's premise was FACTUALLY STALE — the shipped glb has 24
  materials / 13 images / 7 textured (not "13 flat colours, AO only"). The KERNEL (weak surface
  response) survives — but the board showed the single biggest tell at the owner's angles is the
  FLAT LAWN (context), then roof coating character. HANDOFF-2026-07-21.md referenced by the task
  does not exist in the repo (flagged).
- 21 reference images + SOURCES.md in docs/reference-match/research-2026-07-21/ (16 owner
  photos incl. re-pulled 041947 from git history; 5 Wetterbest product/section refs — Clasic
  RAL9005 = matte polyester + metallic fleck + S-wave, 350 mm pitch / 15-20 mm step).
- Reddit STILL blocked to automation (block-page even with browser UA) — forums route used.
- OPTIONS BOARD (progressive stack, each = previous+more) in docs/blender-tasks-done/
  surface-research/: cur baseline · A response-amp (wall rough contrast, roof fleck, clad
  per-board rough; 1469 KB raw) · B + S-wave roof profile (1469 KB) · C + grey_plaster scan
  walls tinted greige + wood grain + damp band (1626 KB ⚠ over raw gate at 512 scans) ·
  D + aerial-grass lawn (1670 KB ⚠). HERO_SHEET + CLOSE_SHEET = the decision images.
  All shot in the real site (5 angles + mobile close-in each, 0 console errors).
- ⚠ Blender session holds stage-D state IN MEMORY; mainfile NOT saved (disk = pre-sprint).
  Next build session: apply the chosen option cleanly from the recipes, budget via 256² scans.

## RESEARCH ROUND 2 — BUDGET TABLE (2026-07-21, measured on stage_D.glb, 512² scans)
Native gltfpack 1.2 (BasisU build, scratchpad) + @gltf-transform/cli. All numbers measured:
| variant | total | textures | geom+json | extReq |
|---|---|---|---|---|
| D raw (png/jpg) | 1670 KB | 476 | 1194 | none |
| D webp (today's ship) | 1316 KB | 191 | 1126 | EXT_texture_webp |
| D meshopt (gltfpack -cc) | 962 KB | 476 | 486 | +quantization+meshopt |
| D KTX2-etc1s | 1021 KB | 300 | 721 | +KHR_texture_basisu |
| D meshopt+etc1s | 786 KB | 300 | 486 | both |
| D KTX2-uastc | 1889 KB ⚠ | 1168 | 721 | (uastc w/o zstd = TRAP, bigger than raw) |
| D gt-meshopt+webp | **861 KB** | 191 | 671 | webp+quantization (NO KTX2) |
- **THE HEADLINE: geometry is our fat, not textures** (1194→486 KB, −59%). Textures are already
  small; webp (191) beats etc1s (300) on wire bytes for our set.
- **KTX2 verdict: PARKED.** Transcoder payload = 570 KB (56 js + 514 wasm) — bigger than
  everything it saves vs webp. Its real win is VRAM (~6-8× less GPU memory) — revisit only if
  texture set grows past ~2 MB or mobile GPU memory becomes the measured bottleneck.
- **D @1024² projection**: webp textures ×3.5 ≈ 670 KB + meshopt geom ≈ 490-670 → **≈1.2-1.35 MB
  = UNDER the gate.** The "realistic vs 1.5 MB" trade IS fake; the unlock is MESHOPT(+webp).
- **extensionsRequired recommendation**: relax the none-rule to allow EXT_texture_webp +
  KHR_mesh_quantization + EXT_meshopt_compression (we control the only loader; MeshoptDecoder
  ships inside three, 28 KB, 2-line wiring + phaseOf child-inheritance caveat). Keep KTX2 out.
- ⚠ TOOLING GOTCHA: gltf-transform REWRITES (decompresses) gltfpack meshopt — pipeline must be
  gltf-transform-only (`meshopt` then `webp`, 861 KB) or gltfpack-last. npm gltfpack has NO
  BasisU (native build needed). uastc needs zstd supercompression to be viable — gltfpack 1.2
  flag set didn't include it.
- NOT finished in round 2 (folded into round 3): anisotropy A/B, site/veg/sky deep-dive,
  per-region photo comparison. Q-11/12/13 filed in docs/QUESTIONS.md ✓. Stale 2026-07-15 scene
  dump neutralized in these notes ✓.

## ROUND 3 SPRINT — 2026-07-22 (full log: docs/blender-tasks-pending/round3-log.md)
Six tracks closed; key artefacts in scratchpad (lmAB_*, tod_*, cam_*, aniso_*), harness at
scripts/realism-score.mjs, lightmap PNGs (1024=697KB png/53KB webp). Blend saved 06:43:34.
Headlines: baked-GI PoC proven (53 KB, rested-crossfade path); golden-hour+window-glow = biggest
zero-byte win; camera tilt/fov = one-line fixes; teardowns show baked lighting is what the
great-looking sites do; realism scores baselined (cur 50 → D 52; ground worst region); aniso
refuted; KTX2 stays parked (round 2). See the ranked list in the round-3 report.
