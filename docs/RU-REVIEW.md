...
| configuratorPage.fence.* (LANE C: fence category price + length estimate — categoryPrice, foundationIncluded, typeNote, lengthLabel/Preset, customLength*, estimate*, lengthHint, cta, disclaimer) | 2026-07-26 | NEEDS NATIVE REVIEW before launch. Machine-drafted RU; unit is "лей/погонный метр" / "пог. м" (linear metre, deliberately distinct from roofs' "м²"); only the owner data point used (жалюзи от 2900 лей/пог. м, фундамент включён), other types "цена по запросу", nothing invented. |
