...
| **Homepage flow — 3D→slideshow handoff** | `feature/project-slideshow` @ `44efe68` (un-parked; LANE A hero merged in) | **Built + previewable, unmerged.** 3D build → settle → copy card → dim → cross-fade into an auto-advancing slideshow of the 4 real projects (Orhei 100 m² / Costești 320 m² / Cahul 180 m² / Chișinău 280 m²), card legible throughout; the canvas **UNMOUNTS after the cross-fade** (draw calls 486→0, scene + shadow map disposed — the 485-draw-call build win untouched). Reduced-motion: static first slide, no auto-advance, no build. `?no3d=1`: no canvas. **⚠️ Photo↔project pairing PROVISIONAL — owner to confirm (Q-14); no years invented.** Does NOT reach `main` until Q-12 (hero direction) is signed off. |
