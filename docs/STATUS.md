...
| **RC-104 Portfolio** | `/portofoliu` | **Partial.** Page ships with 8 real photos, tags, ItemList JSON-LD, sitemap entry — the nav 404 is gone. Not done: filters, per-project detail pages, before/after sliders. **Q-14 partially resolved (2026-07-25): photos in hand; only per-photo metadata (locality/year/m²/work-type) pending — must NOT be invented.** May caption by **visibly-true facts only** (e.g. "Acoperiș din țiglă metalică") until the owner supplies the rest. |
