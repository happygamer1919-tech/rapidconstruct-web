...
| **B4 🚨 LAUNCH-BLOCKING (partial)** | **Q-10 — configurator prices** | **ROOFS ANSWERED 2026-07-25 (owner, montaj inclus, "de la"):** metalică 1100 · șindrilă 1200 · rocă vulcanică 1400 · **ceramică 1600** (now has a band) lei/m² — these REPLACE the Imperlux 450/550/800 competitor bands. **Still open:** (a) **configurator CODE** must be updated to these on `feature/configurator` (LANE C) — the §1b gate clears only then, and ceramic goes into JSON-LD; (b) **fences** — only jaluzele = **2900 lei/METRU LINIAR** (unit ≠ lei/m² — schema needs a per-`ml` field); șipcă/plin/combinat unanswered; (c) 🚨 **"160 lei/m²" advertised vs 1100 real = misleading-advertising** — hard gate added, site must not carry the 160 claim until scope explained. Roof-calculator ceramic (Creaton 57/58) still pending. | **Max** |
