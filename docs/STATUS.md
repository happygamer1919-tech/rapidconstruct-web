...
| **Homepage flow — 3D→slideshow handoff** | `feature/project-slideshow` (un-parked; LANE A hero merged in) — `src/components/HouseBuild.tsx`, `ProjectSlideshow.tsx`, `src/data/slideshow.ts` | **Built + previewable (:3802), unmerged.** Sequence: 3D build → settle → copy card → dim → cross-fade into an auto-advancing slideshow of the **4 real projects** (Orhei 100 m² / Costești 320 m² / Cahul 180 m² / Chișinău 280 m²), card legible throughout. **Canvas UNMOUNTS after the cross-fade** — draw calls 486→0, scene/2048² shadow map disposed. Reduced-motion: static first slide, no auto-advance, no build. `?no3d=1`: no canvas. **⚠️ Photo↔project pairing PROVISIONAL — owner to confirm on preview (Q-14); one-line `src` swap per slide; NO years invented.** |
